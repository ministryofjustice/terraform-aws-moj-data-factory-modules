...
- Entra **Enterprise Application (service principal) Object ID** (not the App registration Object ID or Application client ID)
...
| <a name="input_object_id"></a> [object\_id](#input\_object\_id) | Enterprise Application (service principal) Object ID of the Microsoft Entra identity allowed to assume the IAM role. This is the Object ID found under Entra ID > Enterprise applications > <app> > Overview, NOT the App registration Object ID or the Application (client) ID. It must match the `sub` claim in the OIDC token; supplying the wrong ID causes sts:AssumeRoleWithWebIdentity to fail. | `string` | n/a | yes |
