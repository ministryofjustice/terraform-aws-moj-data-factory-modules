
# Returns the name (ID) of the S3 bucket created by this module.
output "bucket_name" {
  description = "Name of the scanned S3 bucket."
  value       = module.bucket.s3_bucket_id
}

# Returns the Amazon Resource Name (ARN) of the bucket.
output "bucket_arn" {
  description = "ARN of the scanned S3 bucket."
  value       = module.bucket.s3_bucket_arn
}

# Returns the unique identifier of the GuardDuty Malware Protection plan.
output "guardduty_malware_protection_plan_id" {
  description = "ID of the GuardDuty Malware Protection plan."
  value       = aws_guardduty_malware_protection_plan.this.id
}

# Returns the ARN of the IAM role assumed by GuardDuty Malware Protection.
output "guardduty_malware_protection_plan_role_arn" {
  description = "IAM role ARN used by GuardDuty Malware Protection."
  value       = aws_iam_role.guardduty_malware_protection.arn
}

# Returns the current status of the malware protection plan. 
# Typical values include: 
# - ACTIVE
# - WARNING
# - ERROR
output "guardduty_malware_protection_plan_status" {
  description = "Status of the GuardDuty Malware Protection plan."
  value       = aws_guardduty_malware_protection_plan.this.status
}