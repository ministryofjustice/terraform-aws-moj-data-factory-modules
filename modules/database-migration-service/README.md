...
This Terraform module provisions an AWS DMS (Database Migration Service) setup for replicating data from an Oracle or Postgres source database to an S3-based data lake architecture.

It automates the creation and configuration of the following components:

- A DMS replication instance and endpoints
- Oracle/Postgres source configuration (via Secrets Manager)
...
| [aws_iam_role.dms_vpc](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/iam_role) | data source |
...
| <a name="input_dms_replication_instance"></a> [dms\_replication\_instance](#input\_dms\_replication\_instance) | Properties of the dms replication instance to be used in the migration | <pre>object({<br/>    replication_instance_id      = string<br/>    subnet_group_id              = optional(string)<br/>    subnet_group_name            = optional(string)<br/>    subnet_ids                   = list(string)<br/>    allocated_storage            = number<br/>    availability_zone            = string<br/>    engine_version               = string<br/>    kms_key_arn                  = string<br/>    multi_az                     = bool<br/>    replication_instance_class   = string<br/>    inbound_cidr                 = optional(string)<br/>    apply_immediately            = optional(bool, false)<br/>    preferred_maintenance_window = optional(string, "sun:10:30-sun:14:30")<br/>  })</pre> | n/a | yes |
...
| <a name="input_oracle_client_layer_arn"></a> [oracle\_client\_layer\_arn](#input\_oracle\_client\_layer\_arn) | ARN of a Lambda layer providing the Oracle Instant Client libraries. When set, the metadata generator<br/>    Lambda runs python-oracledb in thick mode (required for Oracle Native Network Encryption). The layer must<br/>    expose the client libraries under the path given by oracle\_client\_lib\_dir. When null, oracledb runs in thin<br/>    mode (no Native Network Encryption support). Only relevant when dms\_source.engine\_name is 'oracle'. | `string` | `null` | no |
| <a name="input_oracle_client_lib_dir"></a> [oracle\_client\_lib\_dir](#input\_oracle\_client\_lib\_dir) | Directory inside the Lambda runtime where the Oracle Instant Client libraries are mounted by oracle\_client\_layer\_arn. Ignored when oracle\_client\_layer\_arn is null. | `string` | `"/opt/oracle/instantclient"` | no |
