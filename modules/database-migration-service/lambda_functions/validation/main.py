...
def move_object(bucket_to: str, bucket_from: str, key: str, mutable: bool = False) -> None:
    ...
    logger.info("COPY PARAMS: %s", copy_params)
...
def strip_data_type(data_type: str) -> str:
    ...
    raise MetadataTypeMismatchException(f"'{data_type}' is not valid or is not a supported data type.")
...
def normalise_to_agnostic_type(data_type: str, column_name: Optional[str] = None) -> str:
...
class FileValidator:
    ...
    def execute(self) -> None:
        ...
        if self.errors:
            ...
            location = f"https://s3.console.aws.amazon.com/s3/buckets/{self.fail_bucket}/{self.key}"
            payload = {
    ...
    def _validate_file(self, path: str, fs: Any = fs, validate_column_attributes: bool = True) -> None:
        """
        Parameters
        ----------
        path : [type]
        ...
        if self.parquet_table_name not in self.metadata_s3_keys.keys():
            self._add_error(error=f"'{self.parquet_table_name}' is an not in the list of keys in the metadata bucket")
    ...
    def _validate_column_attributes(
        ...
        for col_name, col_type in zip(parquet_file.schema_arrow.names, parquet_file.schema_arrow.types):
    ...
    def _validate_column_types(
        ...
        for col in sorted(set(metadata_cols).intersection(set(parquet_cols))):
            try:
                agnostic_parquet_type = normalise_to_agnostic_type(data_type=parquet_cols[col], column_name=col)
                agnostic_metadata_type = normalise_to_agnostic_type(data_type=metadata_cols[col], column_name=col)
